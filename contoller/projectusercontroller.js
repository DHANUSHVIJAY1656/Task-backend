...
const assignUserToProject = async (req, res) => {
  ...
  try {
    ...
    if (!creator)
      return res.status(404).json({ message: "Creator (assigner) not found" });

    const assignment = new ProjectUser({
      project_id,
      user_id,
      role_in_project,
      created_by,
    ...
    res
      .status(201)
      .json({ message: "User assigned to project successfully", assignment });
  } catch (error) {
...
const getAllProjectUsers = async (req, res) => {
  try {
    const assignments = await ProjectUser.find().populate(
      "project_id user_id created_by",
      "project_name name email"
    );
...
const getProjectUsers = async (req, res) => {
  ...
  try {
    ...
    const users = await ProjectUser.find({ project_id: projectId }).populate(
      "user_id created_by",
      "name email role"
    );
    ...
    const employees = users.filter(
      (user) => user.role_in_project === "Employee"
    );
